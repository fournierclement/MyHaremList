...
//getUsersByName(req,res,callback) -> callback(req,res,users)
var getUsersByName = function(req,res,callback){
    pg.connect(process.env.DATABASE_URL,function(err,client,done){
        if (err) {
            done(client);
            console.log(err);
        ...
        client.query("SELECT nickname FROM users WHERE nickname ILIKE $1",
            ['%'+req.body.word+'%'],
            function(err,result){
            done(client);
            if(err){
...
var editUser = function(req,res,callback){
    bcrypt.genSalt(10, function(err, salt) {
        bcrypt.hash(req.body.newpassword, salt, function(err, hash) {
            ...
            pg.connect(process.env.DATABASE_URL, function (err,client,done) {
               //Handle connection to psql errors
                ...
                client.query("UPDATE users SET password = $1, email = $2 WHERE nickname = $3",
                    [req.body.newpassword,req.body.newemail,req.body.nickname],
                    function(err,result){
                    done(client);
...
var deleteUser = function(req,res,callback){
    pg.connect(process.env.DATABASE_URL, function (err,client,done) {
        ...
        client.query("DELETE FROM users WHERE nickname = $1",
            [req.body.nickname],
            function(err,result){
            done(client);
            if (err) {console.log(err);return res.sendStatus(500);}
